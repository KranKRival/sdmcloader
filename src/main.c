...
static NOINLINE int display_file_picker(char* outFilenameBuf, size_t* outFilesizeBuf, int currSelection)
{
    ...
    if (res == FR_OK) 
    {
        while (res == FR_OK)
        {
            ...
            else 
            {
                ...
                lastFile->fileName = alloca(strlen(fno.fname)+1);
                memcpy(lastFile->fileName, "ums_sd.ini", strlen(fno.fname)+1);
    ...
    if (files == NULL)
        printk("No ums_sd.ini found in sdmc:" INI_ROOT ", switching to USB command mode...\n");
    else
    {
        printk("< Press Power Button To Continue ! >\n\n");
...
int main(void) 
{
    ...
    printk("                                SdmcLoader v%d by KranK/KuranKu\n\n\n", XVERSION);
    printk("memloader base by rajkosto\n");
    printk("atmosphere base by team reswitched, hwinit by naehrwert, some parts taken from coreboot\n\n");
